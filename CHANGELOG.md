...
## 2026-08-23 — เปลี่ยนข้อมูลอ้างอิงเป็นของจริง / หมวดหมู่ซ่อม auto-match ตามอาการ / เลิกใบเสนอราคาซ้ำซ้อน / บังคับอัพเดทลูกค้าก่อนปิดงาน

**ขอบเขต:** แก้บั๊ค UX หน้าจัดการ Ticket, รื้อ workflow ใบเสนอราคาของช่างไม่ให้ซ้ำซ้อนกับของ Admin, ปรับ UI หน้ารับงานของช่างให้เลือก "ซ่อมเอง"/"ส่งต่อ Partner" ได้ทันที, เพิ่มกฎบังคับอัพเดทลูกค้าก่อนปิดงาน, และเปลี่ยนข้อมูลอ้างอิงทั้งหมด (ลูกค้า/อุปกรณ์/Partner/หมวดหมู่การซ่อม) จากข้อมูลสมมติเป็นข้อมูลจริงของบริษัท พร้อมฟีเจอร์ใหม่ให้ระบบกรองหมวดหมู่การซ่อมตามอาการที่ลูกค้าแจ้งเอง

### 1. บั๊ค: หน้า "จัดการ Ticket" กดเมนูแล้วข้อมูลไม่ตรงกับกด "กรอง"
เดิมมี logic ซ่อน ticket สถานะ `completed`/`closed` ออกโดยอัตโนมัติเมื่อเข้าหน้าตรงๆ (ไม่มี query string) แต่ dropdown สถานะดันค้างคำว่า "ทุกสถานะ" อยู่ดี ทำให้ดูเหมือนบั๊ค (ต้องกดปุ่ม "กรอง" ก่อนข้อมูลถึงจะครบ)
**แก้:** ตัด logic `filtered`/`hide_done` ทิ้งทั้งหมด — กดเมนูตรงๆ โชว์ทุกสถานะเหมือนกด "กรอง" เป๊ะ แต่เรียงงานที่ยังไม่จบไว้บนสุด (`ORDER BY FIELD(status, ...)`) งาน `completed`/`closed` จมท้ายตารางแทนที่จะซ่อนไปเลย

### 2. บั๊ค: หน้าเว็บ cache ทำให้เห็นข้อมูลเก่าค้าง
ไม่มีการตั้ง Cache-Control header ที่ไหนในระบบเลย เบราว์เซอร์เลยเก็บหน้า dashboard ไว้ cache บางจังหวะ
**แก้:** `MY_Controller::render()` ตั้ง `Cache-Control: no-store, no-cache, must-revalidate` + `Pragma: no-cache` ให้ทุกหน้าทุก role ที่ผ่าน `render()`

### 3. เลิกใบเสนอราคาซ้ำซ้อนระหว่างช่างกับ Admin + ล็อกราคาแยกตามที่มา
เดิม Admin ต้องออกใบเสนอราคาของตัวเองอีกใบเสมอ (สร้างแถวใหม่ในตาราง `quotations`) ถึงจะมีปุ่มส่งลูกค้าได้ ทั้งที่ยอดเท่าใบของช่างเป๊ะ — กลายเป็นมี 2 เอกสารซ้ำกันโดยไม่จำเป็น
**แก้:** แยกกฎตามที่มาของใบเสนอราคาเดิม (`partner_id` เป็นตัวบอกว่าใครส่ง — ว่าง = ช่างในบริษัท, มีค่า = Partner):
- **ช่างในบริษัทส่งเอง** → ไม่ต้องออกใบซ้ำอีกใบ Admin ตรวจสอบ/แก้ข้อความได้จากการ์ดใบต้นฉบับโดยตรงใน `admin/tickets/detail.php` แล้วกดส่งลูกค้าได้เลย แต่ **ห้ามแก้ยอดเงิน** ล็อกไว้เป๊ะเท่าที่ช่างเสนอ (`abs(quote_amount - partner_quote_amount) < 0.01` ทั้งฝั่งเซิร์ฟเวอร์และ SweetAlert กันฝั่ง client) ถ้าราคาผิดต้องให้ช่างแก้แล้วส่งใหม่เท่านั้น
- **Partner ส่งมา** → ยังคงต้องออกใบในนามบริษัทแยกต่างหากเหมือนเดิม (เอกสาร 2 ใบคือของตั้งใจ เพราะเป็นการบวกราคาจาก vendor ภายนอก) ราคาเป็นแค่ "ขั้นต่ำ" บวกเพิ่มได้ ไม่ล็อกเป๊ะ
- แก้ label "ใบเสนอราคาจาก Partner" ที่ hardcode ผิดมาตลอด (ช่างส่งเองก็ขึ้นคำว่า Partner) ให้บอกชื่อผู้ส่งจริงแทน กวาดคำว่า "Partner" ออกจากทุกจุดที่เกี่ยวกับใบของช่าง (`admin/tickets/detail.php`, `modal_detail.php`, `quotation.php`, `views/quotation/partner_view.php`) — ไฟล์หลังนี้แยก layout/หัวเอกสารสำหรับ Partner ออกจาก view เดิมด้วย เพราะ 2 role มีที่มาต่างกันจริง

### 4. หน้ารับงานของช่าง: แยก 2 ทางเลือกชัดเจนตั้งแต่แรกเห็นงาน
เดิมช่างต้องกดรับงานก่อนถึงจะเห็นตัวเลือก "ส่งต่อ Partner" — ถ้าซ่อมเองไม่ได้ตั้งแต่แรกจะไม่มีทางออก
**แก้:** สถานะ `assigned` โชว์ 2 การ์ดคู่กันทันที — "✅ ตัวเลือกที่ 1 — ซ่อมเอง" (เลือกหมวดหมู่การซ่อม) กับ "ส่งต่อให้ Partner (ซ่อมเองไม่ได้)" พร้อมกัน และย้ายฟอร์มใบเสนอราคา (กรณีหมดประกัน) ไปโผล่**หลัง**จากเลือกหมวดหมู่แล้วเท่านั้น แทนที่จะบังคับกรอกใบเสนอราคาก่อนโดยไม่มีโอกาสได้ตัดสินใจอะไรเลย

### 5. บังคับอัพเดทลูกค้าอย่างน้อย 1 ครั้งก่อนกดเสร็จสิ้นงาน
`technician/Tickets.php::complete()` เช็ค `ticket_updates` ของ ticket นั้นก่อนเสมอ (ทั้งกรณีในและหมดประกัน) ถ้ายังไม่เคยส่งอัพเดทรูป/ข้อความให้ลูกค้าเลยสักครั้ง จะกดปิดงานไม่ได้ กันช่างข้ามขั้นแจ้งความคืบหน้า

### 6. เปลี่ยนข้อมูลอ้างอิงทั้งหมดเป็นข้อมูลจริงของบริษัท
ล้างข้อมูลสมมติทิ้งทั้งหมด (`devices`, `tickets`, `ticket_logs`, `ticket_updates`, `quotations`, `customers`, `partners`, `repair_categories` เดิม 20 หมวด) คงไว้แค่ `users`, `technicians`, `faq` ตามที่ระบุ แล้วใส่ข้อมูลจริงจากเอกสารบริษัท (Excel 2 ไฟล์ที่แนบมา — อ่านด้วย `openpyxl` เพราะ Read tool มาตรฐานเปิด `.xlsx` ไม่ได้):
- ลูกค้าจริง 5 บริษัท, Partner จริง 5 ราย (ชื่อเจ้าของแบรนด์: Urovo, TSC, Zebra, Point Mobile, Unitech)
- อุปกรณ์ 50 เครื่อง (10 รุ่น/ลูกค้า x 5 เครื่อง) — S/N 3 ตัวแรกของแต่ละรุ่นเป็นของจริงจากเอกสาร อีก 2 ตัวสร้างต่อจาก pattern เลขเดิม ผูก `devices.partner_id` ตามเจ้าของแบรนด์จริง
- **สมมติเอง** (ไม่มีในเอกสาร ต้องแก้ทีหลังถ้ามีของจริง): เบอร์โทรลูกค้า, `purchase_date`/`warranty_end` (สุ่มช่วง 2 เดือน–3 ปีที่ผ่านมา + ประกัน 2 ปี)
- ไฟล์ PDF สแกน 81 หน้าที่แนบมาด้วยอ่านไม่ได้ (ไม่มี text layer, เครื่องไม่มี OCR) — ใช้ Excel เป็นแหล่งข้อมูลหลักตามที่ระบุไว้ว่า "เตรียมไว้ครบเลย"

### 7. หมวดหมู่การซ่อมใหม่ + ฟีเจอร์กรองตามอาการอัตโนมัติ (ใหม่)
โละหมวดหมู่เดิม 20 หมวด (ทั่วไป ไม่ตรงกับอุปกรณ์จริง) แทนที่ด้วย 16 หมวดที่เกี่ยวกับ Mobile Computer/Handheld และ Barcode/Label Printer เท่านั้น — ตรงกับอุปกรณ์จริงที่เพิ่งใส่เข้าไปในข้อ 6
- เพิ่มคอลัมน์ `repair_categories.keywords` (คำอาการคั่นด้วย comma) แต่ละหมวดผูกคำอาการที่เกี่ยวข้อง เช่น "จอดับ,จอไม่ติด,จอแตก" → เปลี่ยนจอสัมผัส
- `Repair_category_model::get_matching_categories($issue_desc)` เทียบคำใน `issue_desc` กับ `keywords` แบบ substring match — ช่างเห็น**เฉพาะ**หมวดหมู่ที่ตรงกับอาการที่ลูกค้าแจ้งเท่านั้น ไม่ปนหมวดที่ไม่เกี่ยว
- หมวดหมู่ที่ `keywords` เป็นค่าว่าง (เช่น "ตรวจเช็คทั่วไป") = fallback โผล่เฉพาะตอนไม่มีหมวดไหน match เลย กันเคสตันไม่มีตัวเลือกให้เลือก
- `technician/Tickets.php::detail()` เรียกครั้งเดียว ใช้ผลเดียวกันกับ dropdown ทั้ง 3 จุดในหน้าช่าง (รับงานครั้งแรก, รับงานหลังลูกค้ายืนยันราคา, ปรับหมวดหมู่ระหว่างซ่อม) — มีข้อความสีเขียว/เหลืองบอกสถานะ auto-match กำกับไว้บนฟอร์มด้วย
- แก้ไข/เพิ่มคำ keyword ได้เองที่เมนู Admin "หมวดหมู่การซ่อม" (`admin/repair_categories/form.php`)

### 8. ล้าง catalog "อุปกรณ์/ซอฟต์แวร์ในร้าน" (ตาราง `equipment`) ให้ตรงกับ 5 แบรนด์จริงเช่นกัน
พบระหว่างเช็คหลัง deploy ข้อ 6 ว่าตาราง `equipment` (แคตตาล็อกอุปกรณ์ของร้านเอง — คนละตารางกับ `devices` ที่เป็นอุปกรณ์ของลูกค้า) ยังเป็นข้อมูลเก่า/ทดสอบค้างอยู่ ไม่เคยถูกแตะในข้อ 6 เลย ทำให้หน้า "จัดการอุปกรณ์ในร้าน" มีบางแถวไม่มี Partner กำกับ
**แก้:** ล้าง `equipment` เดิมทิ้งทั้งหมด ใส่ใหม่ 8 รุ่น ตรงกับ 8 รุ่นจริงที่มีในตาราง `devices` (Urovo DT50 / DT50 8GB, TSC TE210, Zebra ZT231 203dpi, Point Mobile PM84 / PM84 (P) / PM452 WIFI, Unitech PA768) ผูก `partner_id` ตามแบรนด์เจ้าของครบทุกรายการ

SQL ทั้ง 2 ไฟล์ของวันนี้ (`reseed_full.sql`, `reseed_equipment.sql`) เก็บไว้ใน `db/2026-08-23_reseed_real_data.sql` และ `db/2026-08-23_reseed_equipment.sql` ตามลำดับ — รันไฟล์แรกก่อนเสมอ (สร้าง partner 5 แบรนด์ที่ไฟล์ที่สองต้องใช้)
