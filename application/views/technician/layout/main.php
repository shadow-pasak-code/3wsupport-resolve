...
<title><?= isset($page_title) ? $page_title . ' — ' : '' ?>ระบบช่าง</title>
...
<div class="flex h-screen overflow-hidden">
    <aside class="w-56 bg-slate-900 flex flex-col shrink-0">
        <div class="px-5 py-5 border-b border-slate-700">
            ...
            <span class="ml-2 text-xs bg-sky-600 text-white px-2 py-0.5 rounded-full">ช่าง</span>
        </div>
        <nav class="flex-1 px-3 py-4 space-y-0.5">
            <?php
            $uri = uri_string();
            $active = strpos($uri, 'tech/tickets') !== false;
            $cls = $active ? 'bg-sky-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white';
            echo "<a href='" . base_url('tech/tickets') . "' class='flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm $cls'>
        ...
        <div class="px-3 py-4 border-t border-slate-700">
            <div class="px-3 mb-2">
                ...
                <p class="text-xs text-slate-400">ช่างเทคนิค</p>
            </div>
